...
int main() {
    ...
    if (fd1 < 0) {
        perror("Failed to open /dev/uio1");
        return -1;
    }

    // Map Screen A and B memory addresses
    volatile uint32_t *screenA = (volatile uint32_t *) mmap(NULL, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd0, 0);
    volatile uint32_t *screenB = (volatile uint32_t *) mmap(NULL, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd1, 0);
