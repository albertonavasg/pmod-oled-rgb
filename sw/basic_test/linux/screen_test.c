...
int main() {   
    ...
    volatile uint32_t *screenA = (volatile uint32_t *) mmap(NULL, 4096, PROT_READ | PROT_WRITE, MAP_SHARED, fd, SCREEN_A_BASE_ADDR);
    volatile uint32_t *screenB = (volatile uint32_t *) mmap(NULL, 4096, PROT_READ | PROT_WRITE, MAP_SHARED, fd, SCREEN_B_BASE_ADDR);
