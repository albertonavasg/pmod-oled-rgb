proc create_ipi_design { offsetfile design_name } {
	...
	set screen_0 [ create_bd_cell -type ip -vlnv xilinx.com:user:screen:2.0 screen_0]
...
set_property -name {xsim.simulate.runtime} -value {100ms} -objects [get_filesets sim_1]
	...
	set offset_file "${bd_path}/screen_tb_include.svh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef screen_tb_include_vh_"
	puts $fp "`define screen_tb_include_vh_\n"
...
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores xilinx.com:user:screen:2.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/screen_tb.sv
...
remove_files -quiet -fileset sim_1 screen_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top screen_tb [get_filesets sim_1]
